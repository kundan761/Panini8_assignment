...
blogRouter.get('/getLatestBlog', async (req, res) => {
    try {
        const blogs = await Blog.find({}).sort({ createdAt: -1 }).populate('createdBy', 'username email');
        res.status(200).json(blogs);
    }
    catch (error) {
        res.status(500).json({ message: 'Internal server error' });
...
blogRouter.get('/getBlogByUser', authMiddleware, async (req, res) => {
    try {
        const blogs = await Blog.find({ createdBy: req.user.userId }).populate('createdBy', 'username email');
        if (!blogs.length) {
...
blogRouter.get('/getBlogByTag/:tag', authMiddleware, async (req, res) => {
    try {
        const blogs = await Blog.find({ tags: req.params.tag }).populate('createdBy', 'username email');
        if (!blogs.length) {
            return res.status(404).json({ message: 'No blog posts found for this tag' });
        }
        res.status(200).json(blogs);
    } catch (error) {
        res.status(500).json({ message: 'Internal server error' });
...
blogRouter.patch('/updateBlog/:id', authMiddleware, async (req, res) => {
    try {
        const { title, content, tags } = req.body;
        const blog = await Blog.findByIdAndUpdate(req.params.id, { title, content, tags }, { new: true });
        if (!blog) {
            return res.status(404).json({ message: 'Blog post not found' });
        }
...
blogRouter.put("/like/:id", authMiddleware, async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Blog post liked successfully", blog });
    } catch (error) {
  ...
  blogRouter.put("/comment/:id", authMiddleware, async (req, res) => {
    try {
      ...
      const newComment = {
        commentedBy: req.user._id, // assuming req.user is set by authMiddleware
        comment: comment,
      };
